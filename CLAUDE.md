...
**Auth is real** (Fase 2 sub-1): `auth.php` holds session + login/logout + guards. Admin & pelanggan log in with email+password (`password_verify` against DB), have separate sessions, and every data page is guarded via the config require-chain (`wajibLoginAdmin()`/`wajibLoginPelanggan()`); login pages are standalone so they stay reachable. Logout = `admin/logout.php` / `portal/logout.php`. The portal "current customer" comes from the session (`idPelangganSaatIni()`), not a hardcoded id. Demo creds: admin `admin@starlite.id`/`admin123`, pelanggan `dwi.anjasmoro@gmail.com`/`pelanggan123`. **Other write actions (add/edit/delete/mark) are still mocks** — real CRUD (INSERT/UPDATE/DELETE + CSRF) is the next sub-project, then portal write (profil, pilih paket).
