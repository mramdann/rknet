...
**Auth is real** (Fase 2 sub-1): `auth.php` holds session + login/logout + guards. Admin & pelanggan log in with email+password (`password_verify` against DB), have separate sessions, and every data page is guarded via the config require-chain (`wajibLoginAdmin()`/`wajibLoginPelanggan()`); login pages are standalone so they stay reachable. Logout = `admin/logout.php` / `portal/logout.php`. The portal "current customer" comes from the session (`idPelangganSaatIni()`), not a hardcoded id. Demo creds: admin `admin@starlite.id`/`admin123`, pelanggan `dwi.anjasmoro@gmail.com`/`pelanggan123`. **Admin CRUD is real** (Fase 2 sub-2): write actions post to dedicated handlers `admin/aksi-<entitas>.php` (paket/area/notifikasi/pelanggan/transaksi/lead/pengaturan) using Post-Redirect-Get. `aksi.php` provides CSRF (`tokenCsrf()`/`cekCsrf()`) + flash (`setFlash()`/`tampilFlash()`, rendered in `admin/partials/shell-open.php`). Paket/area/notifikasi support delete (with `confirm()`); pelanggan is toggle-only (FK to tagihan); deleting a paket in use is caught and flashed. Admin password change uses `password_verify` + `password_hash`. **Remaining: portal write** (edit profil, pilih/ubah paket) — the last Fase 2 sub-project. Portal write actions are still mocks until then.
