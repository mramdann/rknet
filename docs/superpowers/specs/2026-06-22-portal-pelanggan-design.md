# Portal Pelanggan Starlite — UI Design (Improve)
...
Membangun portal pelanggan Starlite/Weave versi improve berdasarkan video demo, memakai PHP native + Bootstrap 5, gaya Modern Biru (konsisten dengan landing page).

## Constraints & Decisions

- **Branding:** Sama dengan landing — logo Starlite + Weave, PT Integrasi Jaringan Ekosistem.
...
- **Server:** XAMPP port 8282 → `http://localhost:8282/starlite/portal/...`.
...
- `$paketAktif`: nama (Paket 200 Mbps Starlite), kecepatan, harga (100000), masaAktif (s/d Juli 2026), status.
...
1. **Login** — kartu tengah, logo Starlite+Weave, field No. HP + PIN, tombol biru "Login" → dashboard, footer perusahaan.
...
UI statis → buka tiap halaman di `http://localhost:8282/starlite/portal/...`, cek render, navigasi antar halaman, offcanvas notif, responsif (sidebar jadi offcanvas di mobile).
